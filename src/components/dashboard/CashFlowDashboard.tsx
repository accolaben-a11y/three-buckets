...
interface ClientData {
  ...
  nest_egg_accounts: Array<{ monthly_draw_cents: number }>
  income_items: Array<{
    type: string
...
export default function CashFlowDashboard({
  ...
  const target = scenario.target_monthly_income_cents
  ...
  const b3Max = calcResult?.hecm
    ? (client.home_equity?.hecm_payout_type === 'tenure'
        ? (calcResult.hecm.tenureMonthlyCents ?? 0)
        : scenario.bucket3_draw_cents + 50000) // allow some headroom for LOC
    : 0
  ...
  return (
    <div className="p-5 space-y-4">
      {/* ── MORTGAGE FREEDOM BANNER ── */}
      {showMortgageBanner && (
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl p-5 shadow-lg">
          <div className="text-center">
            <div className="text-sm font-semibold text-green-100 uppercase tracking-wider mb-1">
              Monthly Cash Flow Freed by Eliminating Mortgage
            </div>
            <div className="text-4xl font-bold mb-1">{formatCents(mortgageFreed)}/month</div>
            <div className="text-green-200 text-sm">
              That&apos;s {formatCents(mortgageFreed * 12)}/year — before drawing from any bucket
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        ...
        {!calcLoading && (shortfall > 0 || surplus > 0) && (
          <div className={`mt-3 rounded-lg px-3 py-2 text-sm font-medium ${
            shortfall > 0 ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-yellow-50 text-yellow-700 border border-yellow-200'
          }`}>
            {shortfall > 0 ? `⚠ Shortfall: ${formatCents(shortfall)}/month` : `↑ Surplus: ${formatCents(surplus)}/month`}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <BucketSlider
          ...
          disabled={client.home_equity?.hecm_payout_type === 'lump_sum' || !client.home_equity || client.home_equity.hecm_payout_type === 'none'}
        />
      </div>
    ...
    <div className={`${c.light} border ${c.border} rounded-xl p-3 ${disabled ? 'opacity-60' : ''}`}>
      ...
      {disabled && bucket === 3 && (
        <div className="mt-1.5 text-xs text-slate-500 italic">
          Lump sum payoff — see freed cash flow above
        </div>
      )}
    </div>
  )
