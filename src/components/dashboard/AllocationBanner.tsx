...
export default function AllocationBanner({ surplusByAge, acknowledgedKeys, onOpenDrawer }: Props) {
  ...
  const allSurplusRanges = groupAges(surplusByAge, false)
  const surplusRanges = allSurplusRanges.filter(r => !acknowledgedKeys?.has(`${r.startAge}-${r.endAge}`))
  const acknowledgedRanges = allSurplusRanges.filter(r => acknowledgedKeys?.has(`${r.startAge}-${r.endAge}`))

  if (shortfallRanges.length === 0 && surplusRanges.length === 0 && acknowledgedRanges.length === 0) return null

  const hasShortfalls = shortfallRanges.length > 0
  const hasUnresolved = surplusRanges.length > 0
  const hasAcknowledged = acknowledgedRanges.length > 0
      ...
      <div className="flex flex-wrap gap-3 text-sm">
        ...
        {hasAcknowledged && (
          <span className="text-blue-700 font-medium">
            ✓ {acknowledgedRanges.length} acknowledged
          </span>
        )}
      </div>
